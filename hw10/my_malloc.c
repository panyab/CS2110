...
static metadata_t *split_block(metadata_t *block, size_t size) {
    metadata_t *newBlock = (metadata_t *) ((uint8_t *) block +(block->size) - size);
    newBlock -> size = size;
...
static void add_to_addr_list(metadata_t *block) {
    ...
    if (address_list == NULL) {
        block->next = NULL; //can switch these two around
        address_list = block;
    } else if (block < temp) {
        block->next = temp;
        address_list = block;
    } else {
        metadata_t *prev = temp;
        temp = temp->next;
        while (temp && (uintptr_t) block > (uintptr_t) temp) {
            prev = temp;
            temp = temp->next;
        }
        block->next = temp;
...
static metadata_t *find_best_fit(size_t size) {
    if (address_list == NULL) {
        ...
        metadata_t *not_best = NULL;
        //not_best->size = -1; see if this works later
        metadata_t *temp = address_list;
        while (temp) {
            if (temp->size == size) {
                return temp;
            } else if (temp->size > size && not_best == NULL) {
                not_best = temp;
                //not_best->size = temp->size; //needed?
            } else if (temp->size > size && temp->size < not_best->size) {
                not_best = temp;
                //not_best->size = temp->size;
            }
            temp = temp->next;
        }
        return not_best;
...
void *my_malloc(size_t size) {
    ...
    if (block->size >= size + TOTAL_METADATA_SIZE + 1) {
        metadata_t *split = split_block(block, size);
        return split + 1;
    } else {
...
void my_free(void *ptr) {
    ...
    metadata_t *right = find_right(freed);
    metadata_t *left = find_left(freed);
